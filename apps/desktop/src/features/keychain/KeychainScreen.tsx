...
export function KeychainScreen() {
  ...
  <div className="min-w-0 flex-1 overflow-y-auto">
    ...
    <div className="mx-auto max-w-375 space-y-8 px-4 py-5 md:px-7">
      <VaultSection title="Keys" view={viewMode}>{filteredKeys.length ? filteredKeys.map(key => <VaultCard key={key.id} selected={draft?.id === key.id} icon={<KeyRound size={19}/>} title={key.name} detail="SSH key reference" onClick={() => { setIdentityDraft(null); setDraft(editKey(key)); }}/>) : q ? <Empty text="No matching keys"/> : <Empty text="No SSH keys" action="Add a key reference" onClick={() => { setIdentityDraft(null); setDraft(blankKey()); }}/>}</VaultSection>
