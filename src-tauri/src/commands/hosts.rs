...
async fn create_key_reference(
    ...
    if let Some(value) = private_key.as_deref().filter(|value| !value.trim().is_empty()) {
        vault::store(
            &mut *transaction,
            vault_state,
            "key",
    ...
    key_references::get(pool, &id)
        .await?
        .ok_or_else(|| crate::errors::LumaError::InvalidInput("key reference creation failed".into()))
}
...
mod tests {
    ...
    async fn failed_key_reference_delete_restores_metadata_and_vault_secret() {
        ...
        let error = delete_key_reference(
            &pool,
            &created.id,
            AtomicWriteFailure::AfterSecretWrite,
        )
